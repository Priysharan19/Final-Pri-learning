...
export function campaignPage({ instagramUsername, keyword, refCode, rewardLabel }) {
  ...
  return shell('Pri Learning · Claim', `
    ...
    <section class="card">
      ...
      <p>This QR identifies the A2Z campaign. Open the tracked Instagram conversation, send the campaign keyword, and receive a one-time code after the A2Z referral is verified.</p>
      <div class="steps">
        <div class="step"><span class="num">1</span><div><b>Open the tracked A2Z DM</b><span>The button carries this QR campaign’s referral ID into Instagram so the system can attribute your claim to A2Z.</span></div></div>
        <div class="step"><span class="num">2</span><div><b>Send <span class="code">${escapeHtml(keyword)}</span></b><span>Pri Learning checks that your Instagram identity entered through the official A2Z referral and has not already redeemed this campaign.</span></div></div>
        <div class="step"><span class="num">3</span><div><b>Show your one-time code</b><span>A2Z staff redeems it once. After redemption, the same Instagram identity cannot claim again.</span></div></div>
        <div class="step"><span class="num">4</span><div><b>Stay connected if you want</b><span>You are welcome to follow @${escapeHtml(instagramUsername)} for Pri Learning updates, but following is optional and does not change reward eligibility.</span></div></div>
      </div>
      <a class="button" href="${dmUrl}" rel="noopener noreferrer">Open A2Z verification DM</a>
      <a class="button secondary" href="${profileUrl}" rel="noopener noreferrer">Visit @${escapeHtml(instagramUsername)} on Instagram</a>
      <p class="note">One redeemed reward is available per Instagram identity for this A2Z campaign. Following @${escapeHtml(instagramUsername)} is optional and is not exchanged for the reward.</p>
      ${legalFooter}
    </section>
  `);
}
...
export function privacyPage() {
  return shell('Pri Learning Promotions · Privacy', `
    ...
    <section class="card legal">
      ...
      <h2>Your choices</h2>
      <p>You can stop messaging or unfollow the Instagram account at any time without affecting a reward already issued or redeemed. To request access, correction or deletion of promotion data, use the <a href="/data-deletion">data deletion instructions</a>. We may need to verify that the request comes from the Instagram account concerned.</p>
