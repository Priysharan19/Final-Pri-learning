...
export function loadConfig() {
  ...
  if (isProduction) {
    ...
    if (config.claimSecret.length < 32) {
      throw new Error('CLAIM_SECRET must be at least 32 characters in production.');
    }
    if (config.staffPin.length < 6) {
      throw new Error('STAFF_PIN must be at least 6 characters in production.');
    }
  }
