...
// package exists because some iPad / Swift Playgrounds installs still open that
// copy directly. A stale copy is dangerous: it can look like the current app
// while running an older handwriting recognizer. Until the duplicate is retired,
// CI requires every Swift source file and Package.swift to be byte-identical.
// ─────────────────────────────────────────────────────────────────────────────
...
function sourceFiles(base, dir = base, out = new Map()) {
  for (const name of readdirSync(dir)) {
    ...
    else if (name.endsWith('.swift') || rel === 'Package.swift') {
...
for (const path of paths) {
  const a = canon.get(path);
  const b = copy.get(path);
...
if (failures.length) {
  ...
  console.error('\nPriLearning.swiftpm is canonical. Sync the duplicate before shipping.');
  process.exit(1);
}
