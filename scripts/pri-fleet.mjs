...
function loadFleet() { return readJson(FLEET_FILE); }
function loadDerivedArtifacts() { return readJson(DERIVED_ARTIFACT_FILE); }
...
function ruleSpecificity(rule) { return rule.pattern.replace(/\*/g, '').length; }
function ownerFor(fleet, file) {
  ...
  if (primaries.size > 1) throw new Error(`ambiguous primary ownership for ${file}: ${tied.map(rule => `${rule.pattern}=>${rule.primary}`).join(', ')}`);
...
function derivedRuleFor(policy, file) {
  ...
  if (found.length > 1) throw new Error(`ambiguous derived artifact policy for ${file}: ${found.map(rule => rule.id).join(', ')}`);
  return found[0] || null;
}
function changedFiles(base = 'origin/main') {
  const text = execFileSync('git', ['diff', '--name-only', `${base}...HEAD`], { cwd: ROOT, encoding: 'utf8' }).trim();
  return text ? text.split('\n').filter(Boolean) : [];
}
function isCollaborativeTest(file, owner, agentId) {
  return Boolean(owner && owner.primary === 'qa-release' && agentId !== 'director' && (file.startsWith('client/test/') || file.startsWith('server/test/')));
}
function hasPrimaryOwnedDerivedSource(fleet, policy, rule, agentId, files) {
  return files.some(file => !derivedRuleFor(policy, file) && (rule.source_patterns || []).some(pattern => matches(file, pattern)) && ownerFor(fleet, file)?.primary === agentId);
}
function isCollaborativeDerivedArtifact(file, owner, agentId, files, fleet, policy) {
  ...
  return Boolean(rule && owner.primary === rule.canonical_owner && rule.requires_primary_owned_source === true && hasPrimaryOwnedDerivedSource(fleet, policy, rule, agentId, files));
}
function guardViolations(fleet, policy, agentId, files) {
  const violations = [];
  for (const file of files) {
    const owner = ownerFor(fleet, file);
    if (!owner) { violations.push(`${file}: unowned`); continue; }
    if (owner.primary === agentId || isCollaborativeTest(file, owner, agentId) || isCollaborativeDerivedArtifact(file, owner, agentId, files, fleet, policy)) continue;
    violations.push(`${file}: primary=${owner.primary}, selected=${agentId}`);
  }
  return violations;
}
function maxRisk(fleet, files) {
  let result = 'R1'; const details = [];
  for (const file of files) {
    const owner = ownerFor(fleet, file); const risk = owner?.risk || 'R4';
...
function validateDerivedPolicy(fleet, policy, ids) {
  ...
  if (!Array.isArray(policy.artifacts) || policy.artifacts.length === 0) return [...errors, 'derived-artifacts must define at least one artifact'];
  const artifactIds = new Set(), patterns = new Set();
  for (const rule of policy.artifacts) {
    if (!rule.id || !/^[a-z0-9-]+$/.test(rule.id)) errors.push(`invalid derived artifact id: ${rule.id}`);
    if (artifactIds.has(rule.id)) errors.push(`duplicate derived artifact id: ${rule.id}`); artifactIds.add(rule.id);
    ...
    if (!Array.isArray(rule.source_patterns) || !rule.source_patterns.length) errors.push(`${rule.id}: source_patterns are required`);
    if (!rule.producer) errors.push(`${rule.id}: producer is required`);
    if (!rule.required_gate_id || !fleet.gates?.[rule.required_gate_id]) errors.push(`${rule.id}: required_gate_id must reference a typed fleet gate`);
    if (rule.requires_primary_owned_source !== true) errors.push(`${rule.id}: requires_primary_owned_source must remain true`);
    for (const pattern of rule.patterns || []) {
      if (patterns.has(pattern)) errors.push(`duplicate derived artifact pattern: ${pattern}`); patterns.add(pattern);
      ...
      else if (owner.primary !== rule.canonical_owner) errors.push(`${rule.id}: ${pattern} canonical owner is ${owner.primary}, expected ${rule.canonical_owner}`);
    }
  }
  return errors;
}
...
function validate(fleet, policy) {
  ...
  if (!Array.isArray(fleet.ownership_rules) || !fleet.ownership_rules.length) errors.push('fleet.ownership_rules is required');
  const ids = new Set();
  for (const agent of fleet.agents || []) {
    if (!agent.id || !/^[a-z0-9-]+$/.test(agent.id)) errors.push(`invalid agent id: ${agent.id}`);
    if (ids.has(agent.id)) errors.push(`duplicate agent id: ${agent.id}`); ids.add(agent.id);
    if (!agent.role || !agent.mission) errors.push(`${agent.id}: role and mission are required`);
    if (!Array.isArray(agent.gate_ids) || !agent.gate_ids.length) errors.push(`${agent.id}: gate_ids are required`);
  }
  for (const required of ['director','handwriting','math-reasoning','platform','security','qa-release']) if (!ids.has(required)) errors.push(`required agent missing: ${required}`);
  let packageScripts = {};
  try { packageScripts = readJson(PACKAGE_FILE).scripts || {}; } catch (error) { errors.push(`package.json invalid: ${error.message}`); }
  for (const [gateId, gate] of Object.entries(fleet.gates || {})) {
    ...
    const npmRun = String(gate.command || '').match(/^npm run ([^\s]+)/); if (npmRun && !packageScripts[npmRun[1]]) errors.push(`${gateId}: npm script '${npmRun[1]}' does not exist in root package.json`);
    const nodeFile = String(gate.command || '').match(/^node ([^\s]+)/); if (nodeFile && !existsSync(path.join(ROOT, nodeFile[1]))) errors.push(`${gateId}: node target '${nodeFile[1]}' does not exist`);
  }
  for (const agent of fleet.agents || []) for (const gateId of agent.gate_ids || []) if (!fleet.gates?.[gateId]) errors.push(`${agent.id}: unknown gate '${gateId}'`);
  ...
  for (const rule of fleet.ownership_rules || []) {
    ...
    if (patterns.has(rule.pattern)) errors.push(`duplicate ownership pattern: ${rule.pattern}`); patterns.add(rule.pattern);
...
function printAgentPrompt(fleet, id) {
  ...
  console.log('Required gates:'); for (const gate of gates) console.log(`- ${gate.id}: ${gate.command} [${gate.platform}, ${gate.severity}, ${gate.evidence_class}]`);
}
function printUsage() { console.log('Pri Learning agent fleet V2\n\nCommands:\n  validate\n  list\n  route <repo-path>\n  prompt <agent-id>\n  guard <agent-id> [base]\n  simulate-guard <agent> <paths…>\n  risk [base]\n  compare-risk <declared> [base]\n  status'); }
...
if (command === 'validate') {
  ...
  const target = args[0]; if (!target) throw new Error('route requires a repository path');
  const owner = ownerFor(fleet, target); const rule = derivedRuleFor(derivedPolicy, target);
  if (!owner) { console.log(JSON.stringify({ path: target, owned: false, risk: 'R4', derived: rule?.id || null })); process.exitCode = 2; }
  else console.log(JSON.stringify({ path: target, owned: true, primary: owner.primary, reviewers: owner.reviewers || [], risk: owner.risk, rule: owner.pattern, specificity: owner.specificity, derived: rule?.id || null, derived_gate: rule?.required_gate_id || null }));
} else if (command === 'prompt') {
  printAgentPrompt(fleet, args[0]);
} else if (command === 'guard' || command === 'simulate-guard') {
  const agentId = args[0]; if (!agentId) throw new Error(`${command} requires agent id`); getAgent(fleet, agentId);
  if (agentId === 'director') { console.error('director is read-only and cannot own changed files'); process.exit(1); }
  const files = command === 'guard' ? changedFiles(args[1] || 'origin/main') : args.slice(1);
  if (!files.length) { console.log(JSON.stringify({ agent: agentId, files: [], allowed: true })); }
  else {
    const violations = guardViolations(fleet, derivedPolicy, agentId, files);
    if (violations.length) { for (const violation of violations) console.error(`ERROR: ${violation}`); process.exit(1); }
    console.log(JSON.stringify({ agent: agentId, files, allowed: true }));
  }
} else if (command === 'risk') {
  console.log(JSON.stringify(maxRisk(fleet, changedFiles(args[0] || 'origin/main')), null, 2));
} else if (command === 'compare-risk') {
  const declared = args[0]; if (!RISK_ORDER.includes(declared)) throw new Error('compare-risk requires R1..R4');
  const actual = maxRisk(fleet, changedFiles(args[1] || 'origin/main'));
  console.log(JSON.stringify({ declared, ...actual }, null, 2));
