...
In addition to expression-level metrics, it reports an oracle-group symbol metric:
we give the symbol head the *ground-truth* stroke-to-glyph groups and ask whether
it can classify those glyphs. This cleanly separates symbol-classification failure
from grouping/AST failure before we spend more time training the wrong component.
"""
...
def _oracle_group_symbol_diagnostic(symbol_logits: torch.Tensor, structure: dict):
    """Classify known glyph groups using pooled per-stroke symbol evidence."""
    ...
    for group in structure.get("groups") or []:
        ...
        if not strokes:
            continue
        idx = torch.tensor(strokes, device=log_probs.device, dtype=torch.long)
        pooled = log_probs.index_select(0, idx).mean(dim=0)
        pred, confidence = _best_real_symbol(pooled)
...
def main():
    ...
    for key in ("writer", "trainSamples", "validationSamples"):
        if recreated.get(key) != split_meta.get(key):
            raise SystemExit(
                f"dev split reproduction mismatch for {key}: "
                f"{recreated.get(key)!r} != {split_meta.get(key)!r}"
            )
    ...
    if len(validation_rows) != len(dataset):
        raise SystemExit("internal validation row/dataset mismatch")
    ...
    with torch.inference_mode():
        for sample_number, (batch, row) in enumerate(zip(loader, validation_rows), 1):
            ...
            dist = edit_distance(hyp.canonical, target)
            total += 1
            exact += ok
            char_errors += dist
            ...
            oracle_rows, oracle_ok, oracle_n = _oracle_group_symbol_diagnostic(
                outputs["symbol_logits"], row.structure
            )
            oracle_symbol_ok += oracle_ok
            oracle_symbol_n += oracle_n
            ...
            sample_diagnostics.append({
                ...
                "exact": bool(ok),
                "editDistance": dist,
                "ambiguous": hyp.ambiguous,
                "confidence": hyp.confidence,
                "symbolConfidence": hyp.symbol_confidence,
                "groupingConfidence": hyp.grouping_confidence,
                "relationConfidence": hyp.relation_confidence,
                "truthGlyphCount": len(row.structure.get("groups") or []),
                "predictedGlyphCount": len(hyp.glyphs),
                "oracleGroupSymbolCorrect": oracle_ok,
                "oracleGroupSymbolTotal": oracle_n,
                "oracleGroupSymbolAccuracy": oracle_ok / max(1, oracle_n),
                "oracleGroupSymbols": oracle_rows,
                "predictedGlyphs": [
                    {
                        "id": g.id,
                        "strokes": list(g.strokes),
                        "symbol": g.symbol,
                        "confidence": g.symbol_confidence,
                        "cx": g.cx,
                        "cy": g.cy,
                    }
                    for g in hyp.glyphs
    ...
    metrics = {
        ...
        "oracleGroupingGlyphSymbolsCorrect": oracle_symbol_ok,
        "oracleGroupingGlyphSymbolsTotal": oracle_symbol_n,
    ...
    print(
        "oracle-group glyph symbol accuracy: "
        f"{100*metrics['oracleGroupingGlyphSymbolAccuracy']:.2f}% "
        f"({oracle_symbol_ok}/{oracle_symbol_n})"
    )
    print(
        f"critical structure exact: {100*metrics['criticalStructureExact']:.2f}% "
        f"({critical_total} samples)"
    )
    ...
    for item in sample_diagnostics:
        print(
            f"  #{item['sample']} target={item['target']!r} pred={item['prediction']!r} "
            f"edit={item['editDistance']} conf={item['confidence']:.3f} "
            f"sym={item['symbolConfidence']:.3f} group={item['groupingConfidence']:.3f} "
            f"rel={item['relationConfidence']:.3f} "
            f"glyphs={item['predictedGlyphCount']}/{item['truthGlyphCount']} "
