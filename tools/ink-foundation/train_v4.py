...
content invariance and style-aware adaptation.  A V4 checkpoint must still pass
writer-disjoint release evaluation before it can replace the production model.
"""
...
from data import (
    BOS_ID, EOS_ID, PAD_ID, VOCAB, canonical_text, corpus_files, decode,
    encode, encode_physical, load_examples, physical_text, point_features,
    rasterize,
)
from model import ModelConfig
from model_v4 import PriInkFoundationV4
from style_augmentation import augmented_strokes
from train import ctc_alignment_loss, edit_distance, seed_everything


class MultiViewInkDataset(Dataset):
    """Two coherent handwriting-style views with identical maths supervision."""
    def __init__(self, examples, split: str, config: ModelConfig, writer_to_id: dict[str, int],
    ...
    def _view(self, strokes, seed: int, augment: bool):
        ...
        raster = rasterize(
            source, self.config.raster_height, self.config.raster_width, augment=False
        )
    ...
    def __getitem__(self, index: int):
        ...
        return {
            "points_a": torch.from_numpy(pa), "valid_a": torch.from_numpy(va),
            "raster_a": torch.from_numpy(ra),
            "points_b": torch.from_numpy(pb), "valid_b": torch.from_numpy(vb),
...
def _masked_token_kl(a: torch.Tensor, b: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
...
def _transfer_v3_or_v4(model: PriInkFoundationV4, path: str) -> dict:
    """Conservatively seed all shape-compatible representation tensors."""
    checkpoint = torch.load(path, map_location="cpu", weights_only=False)
    prior_vocab = checkpoint.get("vocab") or []
    if prior_vocab != VOCAB:
        raise SystemExit(
            "V4 --init currently requires the same vocabulary; retrain/export the seed checkpoint first"
        )
    ...
    for key, value in incoming.items():
        now = current.get(key)
        if now is None or tuple(now.shape) != tuple(value.shape):
            skipped.append(key)
            continue
        # Writer heads are corpus-specific. Never transfer identity classifiers.
    ...
    if int(checkpoint.get("architecture_version") or 0) >= 4 and backbone_missing:
        raise SystemExit(f"V4 -> V4 transfer unexpectedly lost tensors: {backbone_missing[:8]}")
    return {
...
def evaluate(model, loader, device, decode_limit: int = 1024):
    ...
    for batch in loader:
        ...
        logits, _ = model(
            batch["points_a"].to(device), batch["valid_a"].to(device),
        ...
        for pred, truth, writer in zip(
            predictions, list(batch["target_text"]), batch["writer"].tolist()
        ):
            if decoded >= decode_limit:
                break
            ok = int(pred == truth)
...
def main():
    ...
    parser.add_argument("--out", default="tools/ink-foundation/runs/pri-ink-foundation-v4.pt")
    ...
    if counts.get("train", 0) < 1 or counts.get("validation", 0) < 1:
        raise SystemExit("V4 training requires writer-separated train and validation samples")
    ...
    all_writers = train_writers + [w for w in val_writers if w not in set(train_writers)]
    ...
    cfg = ModelConfig(
        ...
        max_tokens=args.max_tokens, architecture_version=4,
    )
    train_ds = MultiViewInkDataset(examples, "train", cfg, writer_to_id, args.seed)
    val_ds = MultiViewInkDataset(examples, "validation", cfg, writer_to_id, args.seed)
    ...
    model = PriInkFoundationV4(
        len(VOCAB), PAD_ID, cfg, writer_classes=len(train_writers),
    ...
    if transfer:
        print(f"seed transfer: {transfer['transferred']} tensors from {args.init}")

    for epoch in range(1, args.epochs + 1):
        model.train()
        sums = {"loss": 0.0, "token": 0.0, "ctc": 0.0, "view": 0.0,
                "content": 0.0, "style_writer": 0.0, "adversary": 0.0}
        batches = 0
        for batch in train_loader:
            ...
            with torch.autocast(
                ...
                a = model.forward_with_aux(
                    batch["points_a"].to(device), valid_a, batch["raster_a"].to(device),
                    adversary_strength=adversary_strength,
                )
                b = model.forward_with_aux(
                    batch["points_b"].to(device), valid_b, batch["raster_b"].to(device),
                    adversary_strength=adversary_strength,
                )
                logits_a, style_writer_a, content_writer_a, ctc_a, _, content_a = a
                logits_b, style_writer_b, content_writer_b, ctc_b, _, content_b = b

                token_loss = 0.5 * (
                    F.cross_entropy(
                        logits_a.reshape(-1, logits_a.shape[-1]), labels.reshape(-1),
                        ignore_index=PAD_ID, label_smoothing=0.03,
                    ) +
                    F.cross_entropy(
                        logits_b.reshape(-1, logits_b.shape[-1]), labels.reshape(-1),
                        ignore_index=PAD_ID, label_smoothing=0.03,
                    )
                )
                ctc_loss = 0.5 * (
                    ctc_alignment_loss(ctc_a.float(), ctc_tokens, ctc_lengths, valid_a) +
                    ctc_alignment_loss(ctc_b.float(), ctc_tokens, ctc_lengths, valid_b)
                )
            ...
            batches += 1
            for key, value in (
                ("loss", loss), ("token", token_loss), ("ctc", ctc_loss),
                ("view", view_loss), ("content", content_loss),
                ("style_writer", style_writer_loss), ("adversary", adversary_loss),
            ):
        ...
        print(
            ...
            f"token={sums['token']/max(batches,1):.4f} ctc={sums['ctc']/max(batches,1):.4f} "
            f"view={sums['view']/max(batches,1):.4f} inv={sums['adversary']/max(batches,1):.4f} "
            f"val-exact={100*val['exact']:.2f}% CER={100*val['cer']:.2f}% "
        ...
        if score > best_score + 1e-5:
            ...
            payload = {
                ...
                "train_writers": train_writers, "epoch": epoch, "validation": val,
                "seed": args.seed, "stage": args.stage, "architecture_version": 4,
            ...
            manifest.write_text(json.dumps({
                "format": "pri-ink-foundation", "version": 4,
                "architectureVersion": 4, "stage": args.stage,
                "evidence": (
                    "synthetic/domain-randomised initialization only"
                    if args.stage == "pretrain" else "writer-disjoint real validation"
                ),
                "decoder": "parallel-output-queries+2d-visual+physical-ctc+writer-invariance",
                "checkpoint": out.name, "config": cfg.to_dict(), "vocab": VOCAB,
                "validation": val, "counts": counts,
                "trainWriterCount": len(train_writers),
                "validationWriterCount": len(val_writers), "seed": args.seed,
                ...
                "holdoutPolicy": "final-holdout is never evaluated by train_v4.py",
    ...
    print("V4 IS NOT PRODUCTION EVIDENCE until writer-disjoint release/generalisation gates pass.")
