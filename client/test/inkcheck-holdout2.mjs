// ─────────────────────────────────────────────────────────────────────────────
// SECOND held-out suite — same writer model as inkcheck-holdout.mjs, disjoint
// seed space.
//
// The first holdout's rule — "treat this as the score, do not tune against
// it" — only survives until someone reads its failures while improving the
// engine. The v8 accuracy work did exactly that (it had to: the misreads ARE
// the diagnosis), which spends holdout #1's independence. This file is the
// replacement: generated by the same writer model over the same forms, but
// from a seed the tuning work never executed. When this one gets studied in
// its turn, add a third.
//
// Usage: node client/test/inkcheck-holdout2.mjs [writers]
// ─────────────────────────────────────────────────────────────────────────────
...
const rng = makeRng(60221023);           // held out from training (20260819), hard (777001),
                                         // holdout #1 (90210) and the v8 dev seed (424242)
...
console.log('\nHeld-out #2: consistent simulated writers, multi-form working\n');
...
console.log(`\n  HELD-OUT-2   exact ${exact}/${lines} (${exactPct.toFixed(1)}%)   chars ${charPct.toFixed(1)}%`);
...
console.log(`\nHELD-OUT-2 SCORE — ${exactPct.toFixed(1)}% lines, ${charPct.toFixed(1)}% chars, worst writer ${(100 * worst).toFixed(0)}%`);
