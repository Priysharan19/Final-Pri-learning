...
for (const chapter of [productChapter, flatChapter, lookupChapter]) {
  assert.deepEqual(chapter.dotpoints, [...NCERT_CLASS8_RATIONAL_DOTPOINTS], 'every product view must expose the full NCERT skill map');
  assert.equal(chapter.covers.length, NCERT_CLASS8_RATIONAL_DOTPOINTS.length, 'one exact generator per NCERT skill');
  assert.deepEqual(uncoveredDotpoints(chapter), [], 'no NCERT Rational Numbers skill may be uncovered');
}
...
for (const id of NCERT_CLASS8_RATIONAL_GENERATOR_IDS) {
  ...
  assert.equal(typeof year8[id], 'function', `${id} must be registered in the lazy Year 8 bank`);
}
...
console.log(`NCERT Class 8 Rational Numbers: ${NCERT_CLASS8_RATIONAL_GENERATOR_IDS.length} generators × 4 levels audited; ${numeric + mcqs} sampled questions passed.`);
