...
// two are linked by opaque ids in the device store; passwords, session tokens and
// provider credentials are never copied into IndexedDB by this module.
...
async function saveAccount(pid, account, patch = {}) {
  ...
  const row = {
    ...
    email: account.email ? String(account.email).slice(0, 254) : null,
    name: account.name ? String(account.name).slice(0, 100) : null,
