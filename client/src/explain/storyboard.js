...
function cleanAction(raw, evidence, context) {
  ...
  if (kind === 'focus_math') {
    ...
    const tokens = Array.isArray(raw.tokens) ? raw.tokens.map(v => text(v, 80)).filter(Boolean).slice(0, 10) : [];
    return { kind, expression, tokens, label: text(raw.label, 120) };
  }
  if (kind === 'show_figure') {
    if (!context?.questionFigure) return null;
    const mode = ['graph', 'geometry', 'calculus', 'statistics', 'figure'].includes(raw.mode) ? raw.mode : 'figure';
    return { kind, mode };
  }
  if (kind === 'checkpoint') {
...
function cleanScene(raw, index, evidence, context) {
  ...
  const lines = Array.isArray(raw.lines) ? raw.lines.map(v => text(v, 360)).filter(Boolean).slice(0, 6) : [];
  const narration = text(raw.narration, 700);
  const actions = (Array.isArray(raw.actions) ? raw.actions : []).slice(0, MAX_ACTIONS).map(action => cleanAction(action, evidence, context)).filter(Boolean);
  return { id: text(raw.id, 80) || `story-${index}`, heading, lines, narration, concept: text(raw.concept, 80) || 'generic', actions };
}
...
export function validateStoryboard(raw, solution, context = {}) {
  ...
  if (!Array.isArray(raw.scenes) || !raw.scenes.length || raw.scenes.length > MAX_SCENES) return { ok: false, reason: 'invalid scene count', storyboard: null };
  ...
  for (const scene of raw.scenes) {
    for (const action of scene?.actions || []) {
      if (action?.kind === 'transform_equation' && (!evidence.has(text(action.before, 240)) || !evidence.has(text(action.after, 240)))) return { ok: false, reason: 'invented equation', storyboard: null };
      if (action?.kind === 'focus_math' && !evidence.has(text(action.expression, 240))) return { ok: false, reason: 'invented focus expression', storyboard: null };
    }
  }
  return { ok: true, reason: '', storyboard: { version: STORYBOARD_VERSION, source: text(raw.source, 40) || 'authored', scenes } };
}
