...
export function buildDeterministicStoryboard(solution, context = {}) {
  ...
  if (!scenes.length && solution?.answerText) {
    scenes.push({
      id: 'result', heading: 'Work to the result', lines: [], narration: 'Work through the verified reasoning to the final result.',
      concept: classify(prompt, !!figure), actions: [],
    });
  }
...
function visualFromAction(action, context) {
  ...
  if (action.kind === 'transform_equation') {
    return { kind: 'transform', before: action.before, after: action.after, operation: action.operation || '', diff: diffMath(action.before, action.after) };
  }
  if (action.kind === 'focus_math') {
    return { kind: 'focus', expression: action.expression, tokens: action.tokens || [], label: action.label || '' };
  }
  ...
  if (action.kind === 'checkpoint') {
    return { kind: 'checkpoint', prompt: action.prompt, answer: action.answer || '' };
  }
  return null;
}
