...
function mergeAlternatives(primary, ...lists) {
  ...
  return [...best.entries()].sort((a, b) => b[1] - a[1]).slice(0, 6)
...
function repairDerivativePrimes(symbols) {
  ...
  for (let i = 1; i < symbols.length; i++) {
    ...
    const p = box4(prev.box), m = box4(mark.box);
    const nextSym = canonical(next?.sym || '');
    ...
    const old = canonical(mark.sym);
    mark.sym = "'";
    mark.conf = Math.min(0.94, Math.max(0.84, Number(mark.conf) || 0));
    mark.alts = mergeAlternatives("'", [{ sym: old, conf: Math.min(0.55, Number(mark.conf) || 0) }], mark.alts);
    mark.approx = false;
  }
  return symbols;
}
...
function assembleSimple(symbols) {
  ...
  const extents = ordered.map(s => box4(s.box)).filter(b => b.h > 1).map(b => b.h);
  ...
  const raised = s => {
    const sym = canonical(s.sym), b = box4(s.box);
    if (NEVER_SUPERSCRIPT.has(sym)) return false;
    return substantial.length >= 2 && b.cy < top + 0.35 * bodyHeight
      && b.y2 < top + 0.72 * bodyHeight && b.h < 0.82 * bodyHeight;
  };
  let out = '', power = false;
...
function summarise(lines, reading) {
  ...
  return { ...reading, lines, text: lines.map(l => l.text).join('\n'), minConf: all.length ? minConf : 1,
    margin: all.length ? margin : 1, weakest };
}

function lineIndexes(line, strokeCount) {
  const set = new Set();
  const add = i => { if (Number.isInteger(i) && i >= 0 && i < strokeCount) set.add(i); };
...
function overlapRatio(a, b) {
  ...
  let hit = 0; for (const i of A) if (B.has(i)) hit++;
...
function strongestNativeStructure(jsSymbol, nativeLine) {
  ...
  for (const n of nativeLine?.symbols || []) {
    ...
    const score = Math.max(overlapRatio(jsSymbol.strokeIdxs, n.strokeIdxs), boxIoU(jsSymbol.box, n.box));
    if (score >= 0.58 && (!best || score > best.score)) best = { native: n, sym, score };
  }
  return best;
}
...
function remapJsSymbol(symbol, localToGlobal, lineIndex, ordinal, nativeLine, overrides) {
  ...
  if (Object.prototype.hasOwnProperty.call(overrides, id)) { out.sym = overrides[id]; out.conf = 1; out.approx = false; }
  return out;
}
...
function readWholeNativeLine(line, lineIndex, strokes, overrides, ctx) {
  ...
  let result;
  try { result = recognize(members, {}, ctx || null); } catch { return null; }
  if (!result?.lines?.length) return null;
  const raw = result.lines.flatMap(l => l.symbols || []);
  if (!raw.length) return null;
  const symbols = raw.map((s, i) => remapJsSymbol(s, indexes, lineIndex, i, line, overrides)).filter(s => s.strokeIdxs.length);
  if (!symbols.length) return null;
  repairDerivativePrimes(symbols);
  ...
  return { ...line, text, symbols, strokeIdxs: indexes, unread: false, hybridCoverage: coverage,
...
function fallbackOwnedGlyphFusion(line, strokes, overrides, lineIndex) {
  ...
  const symbols = line.symbols.map((native, ordinal) => {
    ...
    const b = strokeBox(members); if (!b) return native;
    let js = null; try { js = classify({ strokes: members, box: b, strokeIdxs: indexes }, medianH); } catch { return native; }
    if (!js?.sym) return native;
    const id = stableId(lineIndex, indexes, ordinal), oldSym = canonical(native.sym), jsSym = canonical(js.sym);
    const oldConf = Number(native.conf) || 0, jsConf = Number(js.conf) || 0;
    let sym = oldSym, conf = oldConf;
    ...
    if (jsSym === oldSym) conf = Math.min(0.95, Math.max(oldConf, jsConf));
      ...
      sym = jsSym; conf = Math.min(0.82, Math.max(jsConf, 0.5 * jsConf + 0.25 * oldConf));
      alts = mergeAlternatives(sym, [{ sym: oldSym, conf: Math.min(0.70, oldConf) }], native.alts, js.alts);
    } else if (jsConf >= 0.34) conf = Math.min(oldConf, 0.62);
