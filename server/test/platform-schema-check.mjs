...
try {
  ...
  for (const required of [
    ...
    'entitlement_snapshots','billing_events','classes','class_members','assignments','assignment_submissions',
  ...
  assert.throws(() => db.prepare(`INSERT INTO accounts(id,email,name,password_hash,role,created_at,updated_at)
    VALUES ('acct_2','student@example.com','Duplicate','hash','student',?,?)`).run(now, now), /UNIQUE/i,
  ...
  db.prepare(`INSERT INTO learning_events(id,account_id,device_id,device_seq,kind,entity_id,occurred_at,payload_json,created_at)
    VALUES ('evt_1','acct_1','ipad-a',1,'attempt','q1',?,'{}',?)`).run(now, now);
  ...
  console.log(`PLATFORM SCHEMA — PASS — ${tables.size} tables inspected; identity, append-only event, sync cursor, CMS and entitlement invariants hold.`);
} finally {
  db.close();
}
