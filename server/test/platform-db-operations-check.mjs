...
try {
  ...
  check(migrated.prepare("SELECT value FROM platform_meta WHERE key='schema_version'").get().value === '4', 'schema version advances to 4');
  ...
  check(first.integrity === 'ok' && first.schemaVersion === '4' && first.accounts === 2 && first.bytes > 0, 'backup is verified after VACUUM INTO');
  ...
  check(served.prepare("SELECT value FROM platform_meta WHERE key='schema_version'").get().value === '4' && served.pragma('journal_mode', { simple: true }) === 'wal', 'the restored file opens under the platform in WAL mode');
